...
from .converter_base import BaseConverter, CrossConvert, FactorConverter, MetricConverter
...
BASE_CONVERTERS = (BaseConverter, CrossConvert, FactorConverter, MetricConverter)
...
def find_converter_by_unit(unit):
    for converter in CONVERTERS:
        if converter.convert.can_process(unit):
            return converter
    return None
...
def convert(from_unit, to_unit, value):
    if from_unit == to_unit:
        return value
    from_system = find_converter_by_unit(from_unit)
    if from_system is None:
        raise ConverterError(f"Unit: {from_unit} not found")
    if from_system.convert.can_process(to_unit):
        ...
        if cross_converter is None:
            ...
            inter_base = value if from_unit.lower() in from_system.convert.BASE_UNIT else from_system.convert.to_base(value, from_unit)
