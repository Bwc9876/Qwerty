...
from discord.commands import slash_command, ApplicationContext, Option, permissions
...
class Personal(BaseCog, name="Personal"):
    ...
    async def projector(self, ctx: ApplicationContext,
                        action: Option(str, description="The action to perform", choices=['start', 'stop'])):
        if action == 'start':
            if self.projector_proc is not None:
                await ctx.respond("Already Started")
            else:
                self.projector_proc = await create_subprocess_shell(os.getenv("PROJ_EXEC"), stdout=PIPE,
                                                                    cwd=os.getenv("PROJ_WORK_DIR"))
